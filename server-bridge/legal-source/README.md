...
source of truth is `MigratePro/legal/eula/ServerBridge-EULA.md` (a sibling repo) — it's
...
## Whenever the EULA changes in MigratePro

```bash
./scripts/sync_eula_from_migratepro.sh
```

This copies the updated `.md` in from the sibling `MigratePro` checkout and regenerates
...
MigratePro's copy, which `sync_eula_from_migratepro.sh` exists to prevent.
